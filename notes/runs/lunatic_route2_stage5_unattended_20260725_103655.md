...
## Post-Run Losing-State Differential

- All 272 stratified pre-hit queries reconstructed exactly; 195 were
  base-empty. Independent ablations rescued 16 with 4-pixel spatial
  refinement, 6 with query-time delay support, 24 by removing uncertainty
  growth, 34 by removing all retained uncertainty, and 45 with at least one
  shorter horizon. These sets overlap; 147 had no tested single-factor rescue.
- Only frame 1,804 is a fresh canonical hit. The global query first became
  empty at decision 1,680, 126 frames earlier. The live fallback chose
  `down_right_fast`; the fused oracle preferred `stay`/`up_fast` for 74
  guaranteed modeled frames, and the selected action had a 24-pixel
  diagnostic reserve deficit.
- Offline exact replay over all 195 losing queries changed 42 actions with
  survival-first ordering and 50 with survival plus reserve. Both had zero
  fresh local hard-vector regressions. These are shadow results, not physical
  acceptance.
- Forty-eight whole solves preserved Boolean/fused viable and safe-mask
  parity. Fused induction nevertheless cost `125.31/229.58 ms` median/p95
  versus `76.96/197.99 ms`, so labels remain offline until they can be
  produced after authoritative Boolean publication or query-localized.
- Full evidence: `notes/LOSING_STATE_ROOT_CAUSE_20260725.md` and
  `artifacts/viability_audit/stage5_20260725_103655_root_cause.json`.
