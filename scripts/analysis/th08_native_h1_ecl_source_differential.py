...
SCHEMA = "th08-native-h1-ecl-source-differential-v1"
RAW_SCHEMA = "th08-native-snapshot-rolling-trial-v3"
PROJECTION_SCHEMA = "th08-native-snapshot-collision-control-projection-v7"
EXPECTED_RAW_SHA256 = (
...
DEFAULT_RAW = (
    ...
    / "raw"
    / "th08_model_capsule_h1_94_callbacks_20260730.json"
)
...
def _payload(record: dict[str, object]) -> dict[str, object]:
    _require(record.get("schema") == PROJECTION_SCHEMA, "projection schema drift")
...
def _auxiliary_fire_suffix(
    ...
    periodic_by_slot = {
        int(row["slot"]): row
        for row in root_payload["enemy_periodic_emission_state"]["rows"]
    }
    rows: list[dict[str, object]] = []
    for context in root_payload["enemy_auxiliary_ecl_contexts"]["rows"]:
        _require(
        ...
        for _ in range(32):
            key = (offset, timer)
            _require(key not in visited, "auxiliary control flow loops in one tick")
            visited.add(key)
            instruction = instructions.get(offset)
            _require(instruction is not None, "auxiliary PC is not in static ECL")
            ...
            if opcode == 0x02:
                ...
                _require(int(scratch[2]) == 2, "auxiliary wait is not two ticks")
            ...
            if opcode in DIRECT_FIRE_OPCODES:
                ...
                raise ValueError(
                    f"unsupported auxiliary opcode {opcode:#x} at {offset:#x}"
                )
            offset += instruction.size
        else:
            raise ValueError("auxiliary H1 scan exceeded its instruction bound")
        _require(len(direct_fires) == 1, "auxiliary does not fire exactly once")
        rows.append(
            {
                "slot": slot,
    ...
    _require(len(rows) == 6, "expected six active auxiliary contexts")
    return {
        ...
        "numeric_descriptor_authority": (
            "not_causal_after_first_unknown_rng_consumer"
        ),
    }
...
def build_report(
    *,
    ...
    _require(raw.get("schema") == RAW_SCHEMA, "raw trial schema drift")
    ...
    _require(
        raw["result"]["natural_reference"]["status"]
        == "natural_frame_differential_passed",
        "natural reference mismatch",
    )
    ...
    enabled_periodic = [row for row in periodic_rows if row["enabled"]]
    main_callbacks = [
        row
    ...
    auxiliary_callbacks = [
        row
        for row in root_payload["enemy_auxiliary_ecl_contexts"]["rows"]
        if row["installed_callback"]["function_pointer"]
    ]
    _require(not enabled_periodic, "periodic emitter requires exact stepping")
    _require(not main_callbacks, "main installed callback requires lowering")
    ...
    return {
        ...
        "native_acceptance": {
            ...
            "natural_reference_status": raw["result"]["natural_reference"]["status"],
        },
        "causal_root_layers": {
            "timeline": timeline,
            "periodic_emitter": {
                "status": "exact_no_enabled_emitter",
                "active_enemy_rows": len(periodic_rows),
                "enabled_count": 0,
            },
            "installed_callbacks": {
        ...
        "first_mismatch": {
            ...
            "unknown_pair_index": 0,
            "first_birth_without_causal_root_producer": 1220,
            "known_suffix": {
                "rng_pair_indices": list(range(2, 8)),
                "birth_slots": list(range(1221, 1227)),
                "auxiliary_slots_in_native_order": [0, 1, 2, 4, 6, 7],
                "classification": (
                    "retrospective_alignment_after_unknown_prefix_"
                    "not_predictive_authority"
                ),
            },
            "excluded_root_sources": [
                "stage_timeline_exact_no_event",
                "active_enemy_periodic_emitter_exact_none",
                "main_and_aux_installed_callback_exact_none",
                "due_main_slot8_literal_prefix_contains_no_direct_fire",
            ],
            "reason": (
                "the first RNG pair is consumed before the six causally "
                "lowered auxiliary fires, and birth 1220 uses the next pair; "
                "no retained covered root producer explains that prefix"
            ),
        },
        "promotion": {
            "integrated_h1_status": "UNKNOWN",
            "physical_gate_authorized": False,
            "next_minimum_closure": (
                "identify the pre-enemy-or-pre-aux producer that consumes "
                "RNG pair 0 and emits birth 1220; do not skip RNG calls or "
                "condition on endpoint births"
            ),
        },
    }
