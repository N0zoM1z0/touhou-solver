...
## ECL Control-Flow Gate

- **Observed:** all 5,749 callback rows pass deterministic live metadata
  validation. Exactly 1,442 are complete horizon schedules and 4,307 stop at
  `unsupported_control_flow`; every incomplete prefix is unavailable to
  lowering.
- **Observed:** no callback row stops at the legacy `instruction_limit` or
  `repeated_state` boundary. All 1,308 spell-57 rows stop at control with at
  most 26 inspected instructions. Spell 61, 65, and 73 control boundaries
  are present.
- **Observed:** all 25 phase-end rows validate, closing the late-transition
  runtime class that the older decoded image could not replay.
- **Evidence:** two audit generations are byte-identical at SHA-256
  `e1d89da6cee5aced7a87187bde950a2d3fed2303292a366621134526bc963210`.
- **Authority:** only complete rows may supply callback schedules. This run
  adds no future-event, survival, or action authority.

## Performance Gate

- **Observed:** birth-observer p50/p95/p99/p99.9/max is
  `0.1018/0.2018/0.3326/0.5441/0.7539 ms`. The fixed p95 limit is
  `0.2000 ms`, so B4 remains failed despite no over-2-ms row.
- **Observed:** all 14,126 observations retain Windows cycle attribution.
  No completed GC or endpoint transition occurs. Prepare/native/materialize/
  residual p95/max is respectively `0.0065/0.0940`,
  `0.0593/0.6459`, `0.0823/0.5023`, and `0.0720/0.5382 ms`.
- **Observed:** spell-57 ECL read/lookahead p50/p95/max is
  `0.0636/0.1566/2.6109 ms`, versus the older physical workload's
  `0.2808/0.5360/1.4769 ms`. The p95 reduction is descriptive; the live wall
  interval includes capture, decoding, and scheduling.
- **Decision:** the ECL correctness gate passes, B4 does not. Do not
  run-select the lower maximum or infer a corridor-completion cause without a
  transition witness.
