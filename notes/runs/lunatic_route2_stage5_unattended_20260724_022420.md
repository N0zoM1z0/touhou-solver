...
- Frames 6,810 and 10,993 are genuine sensor gaps: no laser was active and
  nearest-bullet clearance was 43.60 and 13.00 pixels.
- Both occur in nonspell stage play, where the old sensor returned no enemy
  bodies because it only followed the active spell-owner pointer.
- Native enemy management applies the lethal contact gate to every one of 480
  slots. CE-0059 therefore replaces owner-only sensing with a full fixed-pool
  capture and records its read latency separately.
- A repeat physical Stage-5 run must distinguish geometry improvement from
  any extra control delay caused by the roughly 9.8 MiB contiguous read.
