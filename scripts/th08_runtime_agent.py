...
def frscreen_blocks_enemy_clock(
    ...
    ``msg_state >= 0`` is the observed MSG interpreter state.  ``-2`` also
    blocks the enemy-manager counter, but its producer and physical meaning
    remain unresolved and must stay separately visible in telemetry.
    """
...
def capture_input_clock_shadow(reader: ProcessReader) -> dict[str, object]:
    """Capture a read-only interval around the native FRScreen clock gate.

    The capture is intentionally an interval rather than a point.  Player
    displacement within a stable manager frame is evidence, not incoherence.
    Optional shadow read failures are returned as unknown observations instead
    of terminating the live controller.
    """
