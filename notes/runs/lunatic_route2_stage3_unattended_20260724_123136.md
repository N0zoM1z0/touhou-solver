...
- This run did not exercise Reisen callback motion: spells 35/38/42/46/50
  retained 551/565/484/586/308 lookahead rows respectively, with zero future
  callback events and zero attached piecewise bullets.
- Historical complete Stage-3 hit counts are `11, 7, 10, 12, 11, 8`.
  The current result is inside observed variance and second-best; it does not
  support “the callback fix made other stages worse,” but one randomized run
  cannot establish improvement.
- Against run `20260724_075004`, hits changed `11 -> 8`: nonspells `5 -> 4`,
  spell 38 `3 -> 0`, and spell 50 `3 -> 4`. The only concentrated regression
  is spell 50.

### Timing and solver health

- All eight contacts followed an empty queried viability kernel. Kernel-empty
  queries were `2959/5421 = 54.58%`; five contacts were modeled committed-
  prefix collisions and three were exact bullet overlaps.
- Current versus baseline medians were: pool read `12.45 -> 13.21 ms`,
  decode `1.85 -> 5.34 ms`, local plan `22.90 -> 28.06 ms`, and global solve
  `217.71 -> 245.11 ms`. This regression is not caused by the sparse
  callback kernel because Stage 3 supplied it no event hazards.
- Spell 50 carried up to 200 lifecycle lasers. Its local-plan median/p95 was
  `52.83/134.69 ms`, and 98/308 decisions (31.8 percent) reached input issue
  after the maximum modeled six-frame delay. A retained representative replay
  spent roughly half of local planning in per-laser lifecycle geometry.
- The regenerated dossier correctly attributes three spell-50 contacts to
  action lag beyond complete support: frame 26,246 at the hit row, and frames
  26,759/27,421 through their last-alive decisions. The original report showed
  only one because it compared the hit row to nominal delay.

### Epoch and recovery findings

- Frame 11,056 had `snapshot_frame=9254`, `action_lag=1802`. The pool capture
  itself was internally consistent at 9,254..9,255; the native `+1800`
  transition occurred after capture and before input issue. The read-time
  epoch guard therefore could not catch it.
- Within 60 frames before contacts, issued action changed on 85.1 percent of
  adjacent decisions and reversed an axis on 23.0 percent, versus 63.3 and
  12.7 percent outside those windows. This is evidence of unstable empty-
  kernel recovery, not proof of a unique cause.
- Offline removal of the control-reserve heuristic worsened its endpoint
  deficit (median `4 -> 24`) and reduced zero-deficit choices (`28 -> 6`) over
  72 retained pre-hit recovery decisions. The heuristic is active and useful;
  it still cannot certify the intermediate bridge after Boolean viability is
  false.

### Corrected gate

The next Stage-3 spell-50 trial must separately report action-time deadline
misses, suppress every newly selected stale direction, and retain
`action_epoch_discontinuity` if a logical counter jump occurs during planning.
Algorithmically, replace discontinuous endpoint recovery with a game-neutral
robust safety-value experiment whose positive superlevel set is differential-
equivalent to the existing Boolean kernel before using its negative values for
live recovery.
