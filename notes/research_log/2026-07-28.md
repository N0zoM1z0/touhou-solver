...
## 2026-07-28 — Prepared replay-capable auxiliary event delivery

- Added an explicit schema-v4 post-issue service for the first auxiliary
  literal-fire event class. It consumes the existing coherent native batch,
  exposes no process-reader interface, and is available only with auxiliary
  batch tracing plus exact Lunatic Stage-5 runtime-ECL identity. Default runs
  retain the unchanged schema-v3 hash-only record.
- Exact identity now publishes one immutable accepted version only after a
  byte-exact match. Each schema-v4 observation retains raw active-VM and
  saved-frame bytes beside their hashes, while every derived request is bound
  to that version, capture record, target, depth, marker, and PC owner.
- **Observed software failure:** the first package export introduced a
  circular import from the game-neutral lowerer back into the live event
  service. The correction keeps the event service an explicit submodule and
  makes trace delivery depend on a narrow Protocol. Import-order regression
  tests pass.
- Added a strict independent physical auditor split into replay and report
  responsibilities. It reconstructs raw state, hash/version checks,
  fail-closed classification, request-local canonicalization, and exact
  raw-byte-oracle results. Synthetic exact delivery passes; forged raw hash,
  production result, runtime base, and static digest all fail closed.
- Fixed the physical workload and timing limits in
  `notes/research/g5/G5_AUXILIARY_LITERAL_FIRE_CYCLE_RUNTIME_DELIVERY_CONTRACT_20260728.md`.
  This is implementation/synthetic authority only. A supervised Lunatic
  Stage-5 spell-107 run is still required; physical timing, source life,
  realized births, geometry, planning, and action authority remain none.
- Focused Ruff and delivery/replay/import-order suites pass. Complete Linux
  discovery passes 1,011 tests in 10.313 seconds; complete Windows UNC
  discovery passes 1,011 tests with three existing platform skips.
