...
## CE-0169: Controller observation epoch was conflated with runtime program identity

Date: 2026-07-29

Observed symptom:

- Focused schema-v5 Lunatic Stage-5 run
  `lunatic_route2_stage5_unattended_20260729_095849` completed hard no-Bomb
  but produced 20 hits.
- The one exact runtime-ECL identity and preparation occurred at controller
  gameplay epoch 0. All 142 selected spell-107 batches occurred at epoch 3.
- Every event derivation failed closed as `runtime_identity_mismatch`; zero
  request entered the lowerer, cache, or independent replay oracle.
- The frame-2 cold program bind also consumed `5.251 ms` (`5.317 ms` total),
  leaving a possible one-decision trajectory perturbation outside p95
  cadence comparison.

Invalid assumption:

- A controller epoch reset caused by scene/sensor/action discontinuity is an
  instruction-image mutation or program-version change.
- Full runtime instruction-index construction is cheap enough to perform
  after issue without minimizing its effect on later physical decisions.
...
- compact failure inventory
  `artifacts/viability_audit/g5_auxiliary_ecl_event_runtime_delivery_v2_stage5_20260729_095849_failure.json`;
- result note
  `notes/research/g5/G5_AUXILIARY_LITERAL_FIRE_CYCLE_RUNTIME_DELIVERY_V2_STAGE5_FAILURE_20260729.md`.

Root-cause class: control/runtime version composition and issue-loop
contention.

Correction:

- retain schema v5/event v2 as an immutable failed version;
- define program identity by the already accepted runtime-ECL equivalence
  fields, treating controller epoch as acceptance/observation provenance;
- prevalidate and materialize the exact target instruction closure before
  gameplay;
- perform only a tightly bounded runtime-base bind after identity; and
- independently replay/cache-audit requests across multiple observation
  epochs under a new schema.

Regression test:

- schema-v6/event-v3 synthetic and retained replay crosses at least three
  controller epochs with one unchanged exact program identity;
- any stage/route/difficulty/base/digest mismatch still fails closed;
- online preparation is at most `1.000 ms`;
- V1 and V2 evidence remains unchanged and failed.

Live verification: pending two consecutive corrected Stage-5 runs at no more
than ten hits, plus later cross-stage regression checks.

Status: observed physical counterexample; V2 rejected, V3 contracted before
implementation.
