...
- Initial velocity is `(cos(angle) * speed, sin(angle) * speed)`. Collision uses
  ...
  each overlap worth 9. **Corrected 2026-07-31:** the returned ordinary-shot
  subtotal is not capped; the 50 limit applies only to the enemy hit-feedback
  accumulator increment. Type 6 remains active after overlap.
...
- Commented the level override, trigonometric velocity, Bomb damage divisor,
  type-6 piercing branch, and what was then believed to be an ordinary-shot
  frame cap. **Corrected 2026-07-31:** that clamp is feedback-only.
- REA evidence:
