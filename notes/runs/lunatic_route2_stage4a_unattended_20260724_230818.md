...
- Session status: **discarded** (`external_stop`). This preliminary sensor run
  loaded flags2 mask `0x2`; the live probe established the actual bit-index-2
  mask `0x4`, so this run is not strategy evidence.
