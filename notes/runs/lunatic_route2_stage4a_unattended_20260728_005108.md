...
## G5 Exact Root/Capsule Audit

- `--viability-audit` retained 1,884 same-session capsules. The 455,037,221-byte raw trace has SHA-256 `93037d9febe609accd44eb150150088c29610443783a4434328478409fee41b0`.
- The exact audit accepted 12,986 canonical root/capsule joins, rejected 1,613 coverage/query-root mismatches, found zero missing named capsules, and found 5,896 accepted Boolean-empty roots eligible for a 32-frame solve.
- Decision/query/source `600/599/598` completed all 36 no-Bomb complete-mask root actions under the exact held-mask stationary continuation. Its 32-frame witness has margin `0x1.f87dd20000000p+3`; all worst paths replay and native mismatches are zero.
- Future-event coverage is `UNKNOWN` from frame 600, so the result has no physical action authority.
- CE-0141 records that the trace-only builder rooted coverage at `manager_frame` rather than the canonical `query_frame`; checkpoint `d5866c4` fixes future trace records. This run predates the correction and remains the physical counterexample.
- The compact report is `artifacts/viability_audit/g5_complete_mask_stage4a_20260728.json`, content digest `a67bac60da036813a330483a30d9d93bea90097414926518985f4d9504efc6fe`, file SHA-256 `aa76c5424788bd6628fdd275580256153024be9934864f0b392481f0663dfd8b`.
