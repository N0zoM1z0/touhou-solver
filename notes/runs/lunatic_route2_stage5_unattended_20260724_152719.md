...
## CE-0090 Physical Interpretation

- **Observed:** Every one of 2,658 synchronous owner observations used pointer
  `0x0057D2F0`, exactly one enemy stride before the async pool base
  `0x005826C0`. The prior async sensor omitted the boss by address range, not
  merely by timing.
- **Observed:** Spell 115 retained 700 owner bodies: 682 contact-enabled and
  18 anticipatory. Entry geometry was centered at `(192,128)` with half-size
  `(36,24)`. No zero-bullet spell-115 decision placed the player above
  `y=160`, and the former upper-center contact cluster did not recur.
- **Observed:** The only spell-115 hit was frame 41,768 at `(8,432)` with
  1,145 bullets and negative modeled clearance. It belongs to the remaining
  global-kernel-exhaustion problem.
- **Conclusion:** This run accepts the targeted missing-owner correction once.
  It does not establish an overall survival gain: total hits increased from
  16 to 21 under a different RNG/respawn/Power history. A randomized
  non-Stage-5 control remains required.
