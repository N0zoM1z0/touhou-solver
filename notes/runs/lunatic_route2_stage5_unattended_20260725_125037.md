...
## Post-Publication Shadow Interpretation

This run isolates post-publication labels on a separate executor with one
native label worker. It labeled 1,359 policies at `150.43/284.57 ms`
median/p95 with zero Boolean-array parity failures. Authoritative Boolean
solve, first policy age, query age, and expired decisions were
`114.91/425.22 ms`, `4/10` frames, `11/27` frames, and 15. The corresponding
Boolean-only expiry was 14; local read/plan and action lag stayed
`13.08/18.18 ms`, `22.71/42.74 ms`, and `3/5` frames. This accepts delivery
isolation. Labels remained shadow-only, so the 18 RNG-distinct hits are not a
survival A/B result.

Native observed input differed from last issued desired input on 805/7,772
Boolean queries. Among 4,917 completed label queries, this changed state
classification nine times. The retained exact 16-query cohort changed 13
best-action sets and six winning classifications when pending-command state
was added. See
