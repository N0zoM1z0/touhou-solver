...
## Transform Decoder Differential Qualification

- **Observed:** This run used behavior-neutral decoder commit `1a6b73a`.
  Local and global planners still used the legacy linear bullet projection.
  The 12-hit result is therefore a fresh randomized baseline, not transform
  model acceptance.
- **Observed:** The first launch session `20260724_103511` lost foreground
  before gameplay and failed closed. It produced no gameplay trace, left no
  verified TH08 process running, and is retained only as a failed session; it
  is excluded from this accepted run and every comparison baseline.
- Compact transform artifact
  `lunatic_route2_stage5_unattended_20260724_103617.transform_differential.json`
  covers 798 spell-111 decisions. The radius-160 trace retained a median 229
  transform-relevant bullets per decision but only 61.5 percent of the active
  pool.
- All 159,692 retained transform samples had active flags zero and original
  flags `0x00100202`; no adjacent active-stop pair was present.
- **Inference:** This does not establish absence of native stop behavior.
  Bullets can activate and clear a transform outside the player-centered
  trace radius, then enter the trace with only original flags and completed
  stop fields. A same-slot lifecycle differential requires explicit,
  opt-in full-pool transform tracing.
- **Gate:** Repeat Stage 5 with `--trace-transform-runtime`, retain a compact
  same-slot transition artifact, and do not change planner behavior until
  active stop, timer, resume speed, angle, and repeat transitions agree with
  native observations.
