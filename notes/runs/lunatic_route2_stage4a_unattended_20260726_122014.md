...
- The corrected native FRScreen/MSG tracker ran with
  `shadow_no_input_or_epoch_authority`; candidate verification and viability
  audit were disabled. No observation or episode changed the mask, epoch,
  estimator, delay/cadence evidence, policy version, or worker.
- The trace retained 3,116 observations: gate false/true/unknown
  `3031/84/1`. All logged observation reads were valid, one interval was
  unstable/unknown, and signed MSG state `-2` was not observed.
- It produced exactly five semantic episodes for delayed same-frame pulse
  groups `4963 x 4`, `6763 x 34`, `20845 x 6`, `29506 x 8`, and terminal
  `44817 x 21`. `TP/FP/FN = 5/0/0` only against that delayed proxy; the last
  episode is right-censored. Episode serial deltas were
  `76, 626, 124, 158, 357`.
- At frozen manager frame `20845`, held/native input `0x81` moved
  `(66.4939, 399.4991) -> (376.0000, 399.4991)`, `309.5061 px` over
  `2.0556 s`, with serial `+124`. Frame `29506` held `stay`, moved `0 px`
  over `2.6287 s`, and advanced serial 158.
- First-repeat classifications were 3,016 gate-inactive and five active.
  Active first repeats occurred after `32.864/39.270/68.187 ms`
  minimum/median/maximum since manager progress, overlapping inactive repeat
  timing. A hypothetical 50-ms cut occurred on 1,728 inactive and five active
  repeats.
- Logged capture median/p95/max was `0.191/0.490/17.181 ms`. Available
  viability queries remained 8,759/8,914, without the rejected guard's epoch
  starvation; total capture calls and controlled CPU contention were not
  measured.
- Raw JSONL SHA-256:
  ...
  This validates Stage-4A shadow selectivity and segmentation only, not a
  neutralizing write, one-reset semantics, side-effect-free delivery, or live
  input authority. Its first censored endpoint used the first new-frame
  sample; future code now reports the last-known old-frame lower endpoint,
  unit-tested but not physically rerun.
...
Retain a no-write counterfactual for movement neutralization and one immutable
epoch transition, broaden pause/scene/other-stage negatives, and resolve or
exclude signed state `-2` before an explicitly scoped actuation trial. The
planner death ledger remains separate evidence; the `17` hits are not a
causal sensor improvement over `120839`.
