...
## Safety-Value And Deadline Postmortem

This run explicitly enabled the optional 32-frame max-min safety-value
fallback. It is a failed live experiment, not an accepted Stage-3 baseline:
15 hits is worse than all six previously retained complete Stage-3 runs
(`7..12` hits). Randomized stage attempts are not seed-matched, so aggregate
hit count alone does not establish causality.

Observed timing separates the likely mechanisms:

- Global solve median/p95 rose from `245.11/398.44 ms` in run `123136` to
  `300.86/461.06 ms`. The safety-value phase itself cost `49.97/58.70 ms`.
- Overall local timing was nearly unchanged, but the 200-laser spell-50 tail
  regressed sharply: local-plan p95 `134.69 -> 210.37 ms`, full pre-trace p95
  `182.58 -> 258.44 ms`, and action-lag p95 `10 -> 15` frames.
- The safety fallback was active on 2,827 decisions and the chosen action was
  in its max-min set on 2,236. A paired retained-hazard replay changed 168/300
  actions without changing robust-collision or negative-clearance counts.
  Thus the ordering is not shown to be geometrically unsafe; the physical
  failure is consistent with background contention plus randomized route
  variation.

The action-time guard was observably active:

- It suppressed all 96 newly planned actions that had already exceeded their
  delay support; 95 were during spell 50.
- It caught one post-capture epoch jump at issue frame 21,141
  (`source=19337`, `capture=19338`, `post_capture_advance=1803`), released
  movement, and invalidated cached planning state.
- No newly selected direction was injected after a recorded deadline miss.
  Holding the older command remains a fail-closed diagnostic behavior, not a
  proof that the held trajectory is safe.

The safety-value live switch therefore remains disabled by default. Its exact
threshold-equivalence and offline oracle are retained, but any future live
use must fit a measured compute budget and pass seed-repeated physical A/B.

## Retained Local Performance Correction

Profiling spell 50 showed that the local planner expanded lifecycle lasers
into thousands of Python `Laser` objects and immediately unpacked them into
numeric arrays. The fused lifecycle-to-packed path removes that object
round-trip without changing TH08 laser semantics. Retained paired replay
`132007.local_laser_fusion_benchmark.json` reports:

- legacy median/p95 `33.97/56.96 ms`;
- fused median/p95 `23.44/46.52 ms`;
- `1.45x/1.22x` speedup;
- zero action differences and zero complete-`Decision` differences across
  100 spell-50 samples.

A C++ finite-segment micro-kernel was also explored and rejected. It improved
the already-fused whole-decision median only `23.31 -> 22.19 ms` and p95
`42.29 -> 39.09 ms`, while floating-point reduction order changed 1/100
actions. The native branch was removed rather than carrying a second semantic
implementation for a marginal gain.

## Next Correction Gate

Run a different randomized stage with safety value disabled and the fused
laser path active. Treat policy delivery, issue-time validity, and viability
exhaustion as separate gates. Compare per-phase timing and unsupported action
epochs, not only aggregate hit count.
