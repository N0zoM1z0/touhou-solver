...
- Baseline `20260724_033341` and this run both recorded 27 hits. Per-phase
  counts changed from `10/5/4/3/2/3` to `16/6/1/2/1/1` for
  nonspell/spells 57/61/65/69/73; native RNG prevents treating the unchanged
  aggregate as proof of equivalence.
- Sparse sensing reduced operational snapshot age from 11/20 to 5/8 frames
  median/p95. Main-loop read latency improved from 12.72/26.03 to
  11.71/15.32 ms and decision cadence remained 3/4 frames. The architecture
  is accepted for asynchronous observation.
- Stable hit-edge evidence found exact enemy-body overlaps at frames 8,806,
  9,505, 10,483, and 10,794. The colliding pointers at 8,806, 10,483, and
  10,794 were absent from their action snapshots; only frame 9,505's helper
  swarm was visible, with negative pipeline clearance and an already empty
  robust viability set.
- Therefore this run closes the sensor-performance question but not survival:
  three contacts require ECL activation prediction, while frame 9,505
  requires earlier preservation of reachable safe volume.
