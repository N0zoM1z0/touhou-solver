...
This was the first complete Boolean-first label shadow, but label work was
serialized on the same global executor. It labeled 1,303 policies with
`79.05/131.29 ms` median/p95 and zero Boolean-array parity failures. First
Boolean policy age remained `4/10` frames and query age `10/25`, but expired
decisions rose from the Boolean-only comparison's 14 to 34. This scheduler is
rejected by CE-0108; the 20 RNG-distinct hits do not estimate label quality.

Native observed input differed from the last issued desired action on
754/8,077 Boolean queries. Among 5,562 completed label queries, the state
classification changed nine times. The retained exact 16-query cohort changed
13 best-action sets and four winning classifications when pending-command
state was added. See
`artifacts/viability_audit/stage5_20260725_122624_pending_pipeline.json` and
