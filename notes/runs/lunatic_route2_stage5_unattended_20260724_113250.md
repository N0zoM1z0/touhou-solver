...
## Failed Callback-Lookahead Gate

- Spell 111 had native hit edges at frames 39,706 and 41,151. Frame 39,706
  is the sharp model counterexample: pipeline clearance was reported as
  49.42 pixels despite native contact.
- The compact callback differential hashes the raw trace and retains 42,377
  stopped `(phase=0, aux=1, velocity=0)` samples, 83,930 moving
  `(phase=1, aux=0, velocity!=0)` samples, and 3,037 adjacent same-slot
  phase/aux/motion changes.
- **Implementation failure:** all 844 lookahead rows contained
  `timer_elapsed=0`, zero callback events, and zero attached bullets. The run
  therefore exercised the previous linear trajectory model. It cannot be
  used as evidence that callback-aware planning failed or improved survival.
- Root cause is the wrong ECL VM timer offset. The corrected timer is
  root/fraction/integer at `VM+0x04/+0x08/+0x0C`, not the separate wait timer
  near `+0x90`. A real decoded sub-63 regression now predicts its repeating
  stop/resume callbacks before another physical attempt is allowed.
