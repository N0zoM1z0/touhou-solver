...
## Cross-Stage Review And CE-0089

- **Observed:** This was the requested non-Stage-3 architectural control.
  Safety value was disabled, the fused laser projection path was active, the
  run completed, and no Bomb input was emitted. Retained complete Stage-6B hit
  counts are `42,30,18,27`; RNG differs, so 27 is neither an accepted
  improvement nor proof of a regression.
- **Observed performance:** Overall local planning was 24.48/46.69 ms
  median/p95 versus 22.99/45.21 in comparison run `081952`. In laser-heavy
  spell 154, the fused local path improved 41.9/115.1 to 31.8/111.7 ms, but
  global solve time remained 290.61/454.02 ms and the spell still had four
  hits. This physically supports the local object-pipeline optimization, not
  a survival claim.
- **Observed fixed overhead:** Pool decode was 6.15/16.19 ms versus
  2.49/5.58 in `081952`. The callback investigation had made full transform
  queue/stop diagnostics part of every default decision even when gameplay
  consumed only tag flags, active flags, and callback state.
- **Inferred algorithm defect, confirmed offline:** Soft boundary reserve
  participated in beam pruning before the uncertain-delay first-action
  certificate was calculated. Paired replay showed enabled reserve could
  increase hard robust-collision and negative-certificate counts. CE-0089 now
  precomputes and reuses those certificates before every soft pruning term.
  Corrected 300-row enabled/disabled counts are equal at 22 robust collisions
  and 36 negative certificates; corrected 213-row pre-hit counts are equal at
  56 and 72.
- **Implemented performance correction:** Default planning decode uses
  consolidated scalar reads below 512 active bullets and numeric gathered
  fields above it. Full queue/stop runtime remains available only through
  explicit `--trace-transform-runtime`. The retained synthetic benchmark has
  exact gameplay-field parity and 1.25x--2.24x median speedup across
  200--1,200 active bullets.
- **Acceptance boundary:** Neither replay nor decode timing erases the 27
  physical hits. A randomized physical run must exercise the corrected
  hard-before-soft ordering; aggregate hit count alone remains insufficient.
